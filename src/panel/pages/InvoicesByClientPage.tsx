...
export const InvoicesByClientPage = () => {
    const { loading, meta, invoices, getAllInvoicesByClient } = usePanelHook();
    const [ searchParams ] = useSearchParams();
    const client_ID = searchParams.get("client_ID") || '0';

    const [page, setPage] = useState(() => {
        const storedPage = sessionStorage.getItem('invoicesClientPage');
        return storedPage ? parseInt(storedPage) : 1;
    });
    
    const [limit, setLimit] = useState(() => {
        const storedLimit = sessionStorage.getItem('invoicesClientLimit');
        return storedLimit ? parseInt(storedLimit) : 18;
    });
    
    const handleLimitChange = (newLimit: number) => {
        setLimit(newLimit);
        sessionStorage.setItem('invoicesClientLimit', newLimit.toString());
    };
    
    const handlePageChange = (newPage: number) => {
        setPage(newPage);
        sessionStorage.setItem('invoicesClientPage', newPage.toString());
        window.scrollTo(0, 0);
    };

    useEffect(() => {
        getAllInvoicesByClient(limit, page, client_ID)
    }, [page, limit, client_ID])
